...
function Sunburst() {
  return (
    <svg
      ...
      aria-hidden="true"
      className="absolute right-[-60px] top-1/2 -translate-y-1/2 w-[520px] h-[520px] z-0 pointer-events-none"
    >
  ...
  return (
    ...
    >
      ...
      >
        {/* Angled divider edge */}
        <div
          className="absolute right-0 top-0 bottom-0 z-[3] pointer-events-none"
          style={{
            width: '96px',
            background: '#EEF2FA',
        ...
        {/* Body */}
        <p style={{ fontSize: '14.5px', lineHeight: 1.75, color: '#2a3a5e', maxWidth: '340px', marginBottom: '2.5rem' }}>
        ...
        {/* Open now badge */}
        <div
          className="inline-flex items-center gap-2.5 mt-10 self-start"
      ...
      >
        ...
        {/* Atomic orbit ring */}
        <div
          className="absolute bottom-[80px] left-[30px] z-[1]"
          style={{
        ...
        {/* Bowtie accent */}
        <svg
          className="absolute top-[40px] left-[50px] z-[1]"
        ...
        <div className="relative z-[2]" style={{ width: '300px', height: '420px' }}>
      ...
      {/* ── STATS BAR ── */}
      <div
        className="absolute bottom-0 left-0 right-0 z-[5] flex"
        style={{ background: '#0E2554', gridColumn: '1 / -1' }}
      >
        {STATS.map((s, i) => (
          <div
            key={s.label}
      ...
      <style>{`@keyframes mcm-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.3; } }`}</style>
    </section>
  );
}